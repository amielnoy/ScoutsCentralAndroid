...
public class SupabaseService {
  ...
  public List<Scout> fetchScouts() throws IOException {
    ...
    for (JsonElement element : historyJson) {
      ...
      if (scoutId == null || activityId == null) continue;
  ...
  public void syncActivities(List<Activity> activities) throws IOException {
    ...
    JsonArray payload = new JsonArray();
    for (Activity activity : activities) payload.add(toActivityJson(activity));
  ...
  public void syncAnnouncements(List<Announcement> announcements) throws IOException {
    ...
    JsonArray payload = new JsonArray();
    for (Announcement announcement : announcements) payload.add(toAnnouncementJson(announcement));
  ...
  public void deleteScout(String id) throws IOException {
    executeRequest(requestBuilder("/scouts?id=eq." + id).delete().build());
  }
  ...
  private static class ActivityMeta {
    ...
    final Instant date;
    ActivityMeta(String id, String title, Instant date) { this.id = id; this.title = title; this.date = date; }
  }

  public List<String> fetchAttendanceForActivity(String activityId) throws IOException {
    JsonArray attendanceJson = getJsonArray("/activity_attendance?select=scout_id&activity_id=eq." + activityId + "&present=eq.true");
    List<String> presentIds = new ArrayList<>();
    for (JsonElement element : attendanceJson) {
      String scoutId = getString(element.getAsJsonObject(), "scout_id");
      if (scoutId != null) presentIds.add(scoutId);
    }
    return presentIds;
  }

  public String getScoutAttendanceHistory(String scoutId, String from, String to) throws IOException {
      try {
          HttpUrl url = Objects.requireNonNull(HttpUrl.parse(restUrl + "/rest/v1/activity_attendance")).newBuilder()
                  .addQueryParameter("select", "*,activities(title,date)")
                  .addQueryParameter("scout_id", "eq." + scoutId)
                  .build();
          Request request = requestBuilderForUrl(url).get().build();
          try (Response response = client.newCall(request).execute()) {
              if (!response.isSuccessful()) {
                  Log.e(TAG, "Supabase request failed: " + response.code() + " " + response.message());
                  return "[]";
              }
              return response.body().string();
          }
      } catch (IOException e) {
          Log.e(TAG, "Error in getScoutAttendanceHistory", e);
          throw e;
      }
  }

  public Instructor authenticateInstructor(String email, String password) throws IOException {
    HttpUrl url = HttpUrl.parse(restUrl + "/rest/v1/instructors").newBuilder()
            .addQueryParameter("select", "id,name,email,password")
            .addQueryParameter("email", "ilike." + email.trim())
            .build();
    Request request = requestBuilderForUrl(url).get().build();
    try (Response response = client.newCall(request).execute()) {
        if (!response.isSuccessful()) throw new IOException("HTTP " + response.code());
        JsonArray instructors = JsonParser.parseString(response.body().string()).getAsJsonArray();
        if (instructors.size() == 0) return null;
        ...
        return new Instructor(getString(row, "id"), getString(row, "name"));
    }
  }

  public Instructor authenticateInstructorByEmailOnly(String email) throws IOException {
    HttpUrl url = HttpUrl.parse(restUrl + "/rest/v1/instructors").newBuilder()
            .addQueryParameter("select", "id,name,email")
            .addQueryParameter("email", "ilike." + email.trim())
            .build();
    Request request = requestBuilderForUrl(url).get().build();
    try (Response response = client.newCall(request).execute()) {
        String body = response.body() != null ? response.body().string() : "[]";
        if (!response.isSuccessful()) throw new IOException("Supabase error " + response.code() + ": " + body);
        JsonArray instructors = JsonParser.parseString(body).getAsJsonArray();
        if (instructors.size() == 0) return null;
        JsonObject row = instructors.get(0).getAsJsonObject();
        return new Instructor(getString(row, "id"), getString(row, "name"));
    }
  ...
  public static class Instructor {
    ...
    public Instructor(String id, String name) { this.id = id != null ? id : ""; this.name = name != null ? name : ""; }
  }
  ...
  public String generateProgressPlan(Scout scout, String interests, String skills) throws IOException {
    ...
    Request request = functionRequestBuilder("/progress-plan").post(RequestBody.create(gson.toJson(payload), JSON)).build();
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) throw new IOException("Supabase function failed: " + response.code());
      JsonObject json = JsonParser.parseString(response.body().string()).getAsJsonObject();
      return getString(json, "plan");
    }
  }
  ...
  private JsonArray getJsonArray(String path) throws IOException {
    ...
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) throw new IOException("Supabase GET failed: " + response.code());
  ...
  private void postJson(String path, JsonElement body, boolean mergeDuplicates) throws IOException {
    Request.Builder builder = requestBuilder(path).post(RequestBody.create(gson.toJson(body), JSON));
    if (mergeDuplicates) builder.addHeader("Prefer", "resolution=merge-duplicates");
    executeRequest(builder.build());
  }

  private Request.Builder requestBuilder(String path) {
    String cleanPath = path.startsWith("/") ? path : "/" + path;
    return new Request.Builder().url(restUrl + "/rest/v1" + cleanPath)
      .addHeader("apikey", apiKey).addHeader("Authorization", "Bearer " + apiKey).addHeader("Content-Type", "application/json");
  }

  private Request.Builder requestBuilderForUrl(HttpUrl url) {
  ...
  private Request.Builder functionRequestBuilder(String path) {
    String cleanPath = path.startsWith("/") ? path : "/" + path;
    return new Request.Builder().url(restUrl + "/functions/v1" + cleanPath)
      .addHeader("apikey", apiKey).addHeader("Authorization", "Bearer " + apiKey).addHeader("Content-Type", "application/json");
  }

  private void executeRequest(Request request) throws IOException {
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) throw new IOException("Supabase request failed: " + response.code());
    }
  }
  ...
  private ScoutLevel parseLevel(String levelRaw) {
    try { return ScoutLevel.valueOf(levelRaw); } catch (Exception ex) { return ScoutLevel.KEFIR; }
  }
  ...
  private List<String> getStringList(JsonObject obj, String key) {
    ...
    for (JsonElement item : element.getAsJsonArray()) result.add(item.getAsString());
    return result;
  }
}
